...
;(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    // like Node.
    //node环境，不支持严格的commonJS，但是支持类似node这样支持module.exports的类commonJS环境
  ...
  var DEFAULTS = {
    // placement of the popper popper的放置位置
    placement: 'bottom',
   ...
   *
   * 通过 `attr:value` 的形式给 popper 添加属性
   * @param {Array} [popper.attributes] Array of attributes to apply, specify `attr:value` to assign a value to it.

   *父元素的HTML元素或者查询字符串
   * @param {HTMLElement|String} [popper.parent=window.document.body] The parent element, given as HTMLElement or as query string.

   *popper 的内容，可以是文本、HTML或者结点；如果不是文本，应当将 `contentType` 设置为 `html` 或者 `node`
   * @param {String} [popper.content=''] The content of the popper, it can be text, html, or node; if it is not text, set `contentType` to `html` or `node`.

   *如果是 `html` 内容会变当做 HTML 解析；如果是 `node` 会原样插入
   ...
   *
   *Popper.js 根据这个顺序来避免溢出边界，他们会依次检测，
   * 这意味着最后的情况绝对不会溢出（即 right 和 bottom）。
  ...
  function Popper(reference, popper, options) {
    //保存相关元素的引用，如果是JQuery实例，则取[0],激活的原始html节点
